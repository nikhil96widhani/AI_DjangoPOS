...
<!--                <a href="#" class="btn  btn-primary"> <i class="me-1 fa fa-shopping-basket"></i> Add to cart </a>-->
                <a class="btn btn-light WishListBtn" data-id="${data.variation_data[ind].id}"> <i class="me-1 fa fa-heart "></i> Add to wishlist </a>
...
// ../whislist/?type=add&id=${variation_data.id}
...
$(document).on('click', '.WishListBtn', function() {
    add_wishlist($(this).data("id"))
});

async function add_wishlist(id) {
    fetch('/api/user-wishlist/?id='+ id + '&type=add')
        .then(result =>{
            if (!result.ok) {
                console.log("problem")
                return
        ...
        .then(data =>{
            if (data['status'] === "success") {
                // console.log("hi")
                // console.log(id)
                // let product_id = $('.value-input').find(':selected').attr('data-product-id');
            ...
            else if (data['status'] === "user not logged in") {
                let url = window.location.origin + "/store/wishlist/"
                window.open(url)
            }
