...
def add_product_with_variation(request):
    ...
    if request.method == 'POST':
        ...
        variation_data['image'] = request.FILES.get('image')
        print("wadasdasdasd", request.FILES.get('image'))
...
def UserWishlist(request):
    if request.user.is_authenticated:
        ...
        if type is not None:
            ...
            variations = wishlist.items.all()
            variation_data = [ProductVariationSerializer(variation).data for variation in variations]
            return Response(variation_data, status=status.HTTP_200_OK)
...
def WishListCount(request):
    if request.user.is_authenticated:
        wishlist, created = WishList.objects.get_or_create(user=request.user)
        return Response(wishlist.items.count())
    else:
        return Response(0)
