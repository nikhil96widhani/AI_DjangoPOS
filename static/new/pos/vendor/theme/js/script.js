...
function hideSidebarBasedOnSize() {
    if (window.innerWidth < 1800 && window.innerWidth > 768) {
        $('body').addClass('aside-mini');
    }
	else {
        $("body").removeClass("aside-mini");
    }
    window.addEventListener('resize', hideSidebarBasedOnSize);
}

hideSidebarBasedOnSize()
